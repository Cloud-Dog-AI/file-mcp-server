...
* ``read-write`` -> the shared ``user`` baseline PLUS the write permissions a
  demo operator needs to actually *use* the file service (browse + create /
  update files, manage their storage profiles).
...
This mirrors a1/chat-client's ``servers/web_flat_roles.py`` exactly; only the
service-specific *use* permissions differ (file:* / profile:write vs chat:*).
"""
...
def _shared_user_baseline() -> set[str]:
    ...
    (``USER_BASELINE_PERMISSIONS`` + named CONFIG/PROFILES grants) is still
    landing on shared main; until it is published the deployed idam exposes the
    baseline via ``BASELINE_ROLE_PERMISSIONS["user"]`` (currently
    ``{"resources:read"}``). Resolve whichever the installed idam provides so
    this builds against both the current AND the post-a0 idam — never crashing
    on an import of a symbol that has not landed yet.
    """
...
def _shared_write_permissions() -> set[str]:
    ...
    those symbols, contributes nothing extra (the literal file-use permissions
...
# file-mcp use-permissions the read-write operator needs on top of the shared
