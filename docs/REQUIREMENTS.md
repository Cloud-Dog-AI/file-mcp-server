...
| `NF-006` | Python 3.13 runtime contract — the project-local runtime is CPython 3.13 across the Dockerfiles, `.python-version`, `pyproject` `requires-python`, ruff/mypy targets, developer/test docs, local `.venv`, and an import-time preflight that fails closed on Python < 3.13; clears the fixable CPython High CVEs carried by the 3.12 base (W28R-3013) | `internal` | `must` | `W28R-3013` | src/file_mcp_server/_runtime.py; W28R-3008..3021 addendum §Project Runtime Contract; AGENT-LESSONS §6.183 | `UC-001` | `test_ut_runtime_contract` |
