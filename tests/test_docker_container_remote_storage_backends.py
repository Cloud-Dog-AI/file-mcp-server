...
def docker_image() -> str:
    ...
    if os.getenv("FILE_MCP_RUN_DOCKER_REMOTE_STORAGE_TESTS", "0") != "1":
        pytest.skip("Set FILE_MCP_RUN_DOCKER_REMOTE_STORAGE_TESTS=1 to enable remote storage Docker tests")
...
def test_container_remote_storage_backend_over_host_network(docker_image: str, tmp_path: Path, backend: str) -> None:
    ...
    if not host_ca_bundle.exists():
        raise RuntimeError("Host CA bundle not found at /etc/ssl/certs/ca-certificates.crt")
    ...
    try:
        ...
        async def _flow() -> None:
            async with Client(
                ...
                await client.call_tool("move_path", {"src": path_txt2, "dst": f"{base_dir}/moved.txt", "overwrite": True})
                await client.call_tool(
                    "rename_path", {"src": f"{base_dir}/moved.txt", "dst": f"{base_dir}/renamed.txt", "overwrite": True}
                )
                ...
                await client.call_tool(
                    "sed_edit_file",
                    {"path": path_txt, "operations": [{"op": "replace_regex", "pattern": "hello", "repl": "hi"}]},
                )
                ...
                await client.call_tool("b64_decode_to_file", {"path": f"{base_dir}/b64.txt", "data": data, "overwrite": True})

                search_timeout = 30 if backend == "webdav" else 10
                await client.call_tool("search_paths", {"query": run_id, "max_depth": 3, "timeout_s": search_timeout})
                await client.call_tool(
                    "search_content",
                    {"query": run_id, "max_depth": 3, "timeout_s": search_timeout, "max_results": 5},
                )
