...
def _write_runtime_env(tmp_path: Path, *, host_port: int, workspace: Path) -> Path:
    ...
    lines = ["FILE_MCP_API_KEY_PRIMARY=secret", *_common_env_lines(host_port=host_port, root="/workspace")]
    env_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return env_path
...
def test_container_smoke_with_host_network_and_mcp_call(docker_image: str, tmp_path: Path) -> None:
    ...
    (workspace / "hello.txt").write_text("hello from docker host network", encoding="utf-8")
    ...
    try:
        ...
        async def _call_read_file() -> str:
            ...
            async with Client(transport) as client:
                result = await client.call_tool("read_file", {"path": "/workspace/hello.txt"})
...
def test_container_smoke_with_bridge_network_port_publish(docker_image: str, tmp_path: Path) -> None:
    if os.getenv("FILE_MCP_RUN_DOCKER_BRIDGE_TESTS", "0") != "1":
        pytest.skip("Set FILE_MCP_RUN_DOCKER_BRIDGE_TESTS=1 to enable bridge publish validation")
    ...
    env_path = _write_runtime_env(tmp_path, host_port=container_port, workspace=workspace)
    ...
    try:
        health = _wait_for_health(f"http://127.0.0.1:{host_port}/health", timeout_s=40.0)
        assert health["status"] == "ok"
...
def test_container_multi_env_override_changes_root_and_api_key(docker_image: str, tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    host_port = _pick_free_port()
    workspace = tmp_path / "workspace"
    logs_dir = workspace / "logs"
    ...
    env_base.write_text(
        "\n".join(["FILE_MCP_API_KEY_PRIMARY=base-key", *_common_env_lines(host_port=host_port, root="/workspace/team_a")])
        + "\n",
        encoding="utf-8",
    )
        ...
        async def _flow() -> None:
            async with Client(
                StreamableHttpTransport(
                    ...
                    allowed = await client.call_tool("read_file", {"path": "/workspace/team_b/b.txt"})
                    assert "beta" in "\n".join(item.text for item in allowed.content if hasattr(item, "text"))
                    with pytest.raises(Exception):
                        await client.call_tool("read_file", {"path": "/workspace/team_a/a.txt"})
            ...
            with pytest.raises(Exception):
                async with Client(
                    ...
                    await client.call_tool("read_file", {"path": "/workspace/team_b/b.txt"})
...
def test_container_multi_folder_scope_controls_and_audit_logs(docker_image: str, tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    host_port = _pick_free_port()
    workspace = tmp_path / "workspace"
    logs_dir = workspace / "logs"
    ...
    env_base.write_text(
        "\n".join(["FILE_MCP_API_KEY_PRIMARY=base-key", *_common_env_lines(host_port=host_port, root="/workspace")]) + "\n",
        encoding="utf-8",
    )
...
""".lstrip(),
        ...
        async def _flow() -> None:
            async with Client(
                StreamableHttpTransport(
                    ...
                    allowed_a = await client.call_tool("read_file", {"path": "/workspace/team_a/readme.txt"})
                    assert "team-a-data" in "\n".join(item.text for item in allowed_a.content if hasattr(item, "text"))
                    allowed_b = await client.call_tool("read_file", {"path": "/workspace/team_b/public/public.txt"})
                    assert "team-b-public" in "\n".join(item.text for item in allowed_b.content if hasattr(item, "text"))
                    ...
                    with pytest.raises(Exception):
                        await client.call_tool(
                            "write_file",
                            {"path": "/workspace/team_b/private/blocked.txt", "content": "denied"},
                        )
    ...
    assert all(isinstance(event["profile"], str) and event["profile"] == "default" for event in audit_events)
    ...
    tool_call_events = [event for event in audit_events if event["action"] == "tool_call"]
    assert tool_call_events, "expected tool_call audit events with extended metadata"
    assert all(isinstance(event["session_id"], str) and event["session_id"] for event in tool_call_events)
    assert all(isinstance(event["client_ip"], str) and event["client_ip"] for event in tool_call_events)
    assert all(isinstance(event["duration_ms"], (int, float)) for event in tool_call_events)
