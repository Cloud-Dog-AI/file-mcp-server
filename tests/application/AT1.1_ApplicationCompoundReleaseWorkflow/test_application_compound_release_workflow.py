...
def test_application_compound_release_workflow(tmp_path: Path) -> None:
    ...
    state_json.write_text(
        '{"release":{"status":"TODO","version":"1.0.0"}}', encoding="utf-8"
    )
        ...
        async def _flow() -> tuple[dict, dict, dict]:
            async with Client(
                ...
                before_payload = json.loads(
                    "\n".join(
                        item.text for item in before.content if hasattr(item, "text")
                    )
                )
                ...
                md_edit = await client.call_tool(
                    "markdown_set_section_file",
                    {
                        "path": str(notes_md),
                        "heading": "Release",
                        "new_content": "DONE item",
                    },
                )
                md_edit_payload = json.loads(
                    "\n".join(
                        item.text for item in md_edit.content if hasattr(item, "text")
                    )
                )
                ...
                json_edit = await client.call_tool(
                    ...
                    {
                        "path": str(state_json),
                        "json_path": "/release/status",
                        "value": "DONE",
                    },
                )
                json_edit_payload = json.loads(
                    "\n".join(
                        item.text for item in json_edit.content if hasattr(item, "text")
                    )
                )
                ...
                after_payload = json.loads(
                    "\n".join(
                        item.text for item in after.content if hasattr(item, "text")
                    )
                )
