...
def test_remote_storage_backend_end_to_end(tmp_path: Path, backend: str) -> None:
    ...
    extra_env["FILE_MCP_API_KEY_PRIMARY"] = _require(
        env_ctx, "FILE_MCP_API_KEY_PRIMARY"
    )
    extra_env["FILE_MCP_AUTH_HEADER_NAME"] = _get(
        env_ctx, "FILE_MCP_AUTH_HEADER_NAME", "Authorization"
    )
    extra_env["FILE_MCP_AUTH_HEADER_SCHEME"] = _get(
        env_ctx, "FILE_MCP_AUTH_HEADER_SCHEME", "Bearer"
    )
    extra_env["CLOUD_DOG__PROFILES__DEFAULT__STORAGE__BACKEND"] = backend
    ...
    with running_server(
        ...
        async def _flow() -> None:
            async with Client(
                ...
                # diff_files
                await client.call_tool(
                    "write_file", {"path": f"{base_dir}/a.txt", "content": "A\n"}
                )
                await client.call_tool(
                    "write_file", {"path": f"{base_dir}/b.txt", "content": "B\n"}
                )
                diff = await client.call_tool(
                    "diff_files",
                    {"path_a": f"{base_dir}/a.txt", "path_b": f"{base_dir}/b.txt"},
                )
                ...
                after = await client.call_tool(
                    "read_file", {"path": f"{base_dir}/x.json"}
                )
                after_text = "".join(
                    getattr(item, "text", "") for item in after.content
                )
                ...
                b64read = await client.call_tool(
                    "read_file", {"path": f"{base_dir}/b64.txt"}
                )
                b64_text = "".join(
                    getattr(item, "text", "") for item in b64read.content
                )
                assert "hi" in b64_text

                # search params: depth/timeout/max_results
                await client.call_tool(
                    "search_paths", {"query": "hello", "max_depth": 2, "timeout_s": 10}
                )
                await client.call_tool(
                    "search_content",
                    {
