...
def test_streamable_http_accept_compatibility_middleware_patches_json_only_accept() -> None:
    captured = {}
...
def test_root_status_page_shows_google_drive_authorize_button_when_token_missing(tmp_path) -> None:
    sent = []
    prev_config = os.environ.get("FILE_MCP_ACTIVE_CONFIG_PATH")
...
def test_health_middleware_google_drive_page_uses_forwarded_proto() -> None:
    ...
    try:
        ...
        assert sent[0]["status"] == 200
        body = sent[1]["body"].decode("utf-8")
        assert "https://filemcpserver0.cloud-dog.net/admin/google-drive/callback" in body
    finally:
        if previous is None:
            os.environ.pop("FILE_MCP_ADMIN_UI_ENABLED", None)
        else:
