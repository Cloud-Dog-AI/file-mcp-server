...
def _backend_ready(backend: str, env: Mapping[str, str]) -> bool:
    if backend == "webdav":
        return bool(_get(env, "FILE_MCP_WEBDAV_BASE_URL", "") and _get(env, "FILE_MCP_WEBDAV_USERNAME", "") and _get(env, "FILE_MCP_WEBDAV_PASSWORD", ""))
    if backend == "ftp":
        return bool(_get(env, "FILE_MCP_FTP_HOST", "") and _get(env, "FILE_MCP_FTP_USERNAME", "") and _get(env, "FILE_MCP_FTP_PASSWORD", ""))
    if backend == "s3":
        return bool(_get(env, "FILE_MCP_S3_ENDPOINT", "") and _get(env, "FILE_MCP_S3_BUCKET", "") and _get(env, "FILE_MCP_S3_ACCESS_KEY", "") and _get(env, "FILE_MCP_S3_SECRET_KEY", ""))
    if backend == "google_drive":
        return bool(
            (_get(env, "FILE_MCP_GDRIVE_FOLDER_ID", "") or _get(env, "FILE_MCP_GDRIVE_FOLDER_URL", ""))
            and _get(env, "FILE_MCP_GDRIVE_CLIENT_ID", "")
            and _get(env, "FILE_MCP_GDRIVE_CLIENT_SECRET", "")
            and (_get(env, "FILE_MCP_GDRIVE_REFRESH_TOKEN", "") or _get(env, "FILE_MCP_GDRIVE_ACCESS_TOKEN", ""))
        )
    return False


@pytest.mark.parametrize("backend", ["webdav", "ftp", "s3", "google_drive"])
    ...
    with running_server(
        ...
        async def _flow() -> None:
            async with Client(
                ...
                if backend != "s3":
                    await _call_with_retry("create_dir", {"path": base, "parents": True, "exist_ok": True})
                else:
                ...
                await _call_with_retry("write_file", {"path": file_a, "content": f"hello {backend} {run_id}\n"})
                await _call_with_retry("copy_file", {"src": file_a, "dst": file_b, "overwrite": True})
                await _call_with_retry("move_path", {"src": file_b, "dst": moved, "overwrite": True})
                await _call_with_retry("rename_path", {"src": moved, "dst": renamed, "overwrite": True})

                await _call_with_retry("write_file", {"path": json_path, "content": '{"x":1}\n'})
                await _call_with_retry("json_set_file", {"path": json_path, "json_path": "/y", "value": 2, "dry_run": False})
                await _call_with_retry("json_copy_file", {"path": json_path, "from_path": "/y", "to_path": "/z", "dry_run": False})
                await _call_with_retry("json_move_file", {"path": json_path, "from_path": "/z", "to_path": "/moved", "dry_run": False})
                await _call_with_retry("json_merge_file", {"path": json_path, "json_path": "/", "value": {"k": "v"}, "dry_run": False})
                await _call_with_retry("json_get_file", {"path": json_path, "json_path": "/moved"})

                await _call_with_retry("write_file", {"path": yaml_path, "content": "root:\n  a: 1\n"})
                await _call_with_retry("yaml_set_file", {"path": yaml_path, "yaml_path": "/root/b", "value": 2, "dry_run": False})
                await _call_with_retry("yaml_copy_file", {"path": yaml_path, "from_path": "/root/b", "to_path": "/root/c", "dry_run": False})
                await _call_with_retry("yaml_move_file", {"path": yaml_path, "from_path": "/root/c", "to_path": "/root/d", "dry_run": False})
                await _call_with_retry("yaml_merge_file", {"path": yaml_path, "yaml_path": "/root", "value": {"e": 5}, "dry_run": False})
                await _call_with_retry("yaml_delete_file", {"path": yaml_path, "yaml_path": "/root/a", "dry_run": False})
                await _call_with_retry("yaml_get_file", {"path": yaml_path, "yaml_path": "/root/d"})

                await _call_with_retry("write_file", {"path": md_path, "content": "# Title\n\nBody\n"})
                await _call_with_retry(
                    "markdown_set_section_file",
                    {"path": md_path, "heading": "Title", "new_content": "Updated body", "dry_run": False},
                )
                await _call_with_retry(
                    "markdown_set_frontmatter_file",
                    {"path": md_path, "updates": {"owner": "matrix"}, "dry_run": False},
                )
                await _call_with_retry("write_file", {"path": xml_path, "content": "<root><item>1</item></root>\n"})
                await _call_with_retry("xml_set_file", {"path": xml_path, "xpath": "/root/item", "value": "2", "dry_run": False})
                await _call_with_retry("write_file", {"path": html_path, "content": "<html><body><p id='m'>x</p></body></html>\n"})
                await _call_with_retry("html_set_file", {"path": html_path, "selector": "#m", "value": "y", "dry_run": False})
                await _call_with_retry(
                    "sed_edit_file",
                    {"path": file_a, "operations": [{"op": "replace_regex", "pattern": "hello", "repl": "hi"}]},
                )
                ...
                await _call_with_retry("b64_decode_to_file", {"path": f"{base}/decoded.txt", "data": data, "overwrite": True})

                await client.call_tool("list_dir", {"path": base, "recursive": True})
                await client.call_tool("search_paths", {"query": "data", "max_depth": 3, "timeout_s": 10})
                await client.call_tool("search_content", {"query": run_id, "max_depth": 4, "timeout_s": 10, "max_results": 20})
                await client.call_tool("validate_file", {"path": json_path})
                await client.call_tool("diff_files", {"path_a": file_a, "path_b": renamed})
                ...
                with pytest.raises(Exception) as excinfo:
                    await client.call_tool("chmod_path", {"path": file_a, "mode": "0644"})
                assert "Not supported for backend" in str(excinfo.value)

                await _call_with_retry("delete_file", {"path": file_a, "missing_ok": True})
