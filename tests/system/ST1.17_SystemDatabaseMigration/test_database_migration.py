...
def test_st_db_01_migration_upgrade_on_fresh_sqlite(
    ...
    db_path = tmp_path / "file-mcp-st-migration.db"
...
def test_st_db_02_crud_via_session_manager(monkeypatch, tmp_path: Path) -> None:
    db_path = tmp_path / "file-mcp-st-crud.db"
    _configure_sqlite_env(monkeypatch, db_path)

    runtime = initialise_database(force_reinit=True)
    try:
        with runtime.session_manager.session() as session:
            session.add(FilePlatformDbState(service="file-st", status="ready"))

        with runtime.session_manager.session() as session:
            row = session.query(FilePlatformDbState).filter_by(service="file-st").one()
            row.status = "verified"

        with runtime.session_manager.session() as session:
            verified = (
                session.query(FilePlatformDbState).filter_by(service="file-st").one()
            )
