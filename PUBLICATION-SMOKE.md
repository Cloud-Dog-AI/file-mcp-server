...
External local-Docker smoke. Starts the published image with checked-in
Docker defaults and probes the local HTTP surfaces.

```bash
set -euo pipefail
TAG="${TAG:-latest}"
NAME="${NAME:-file-mcp-server-publication-smoke}"
...
docker run -d --name "$NAME" --network host \
  ...
  -v "$PWD/env-docker-defaults:/app/env:ro" \
  "cloud-dog/file-mcp-server:$TAG"
...
probe_url http://127.0.0.1:8080/mcp/tools
probe_url http://127.0.0.1:8080/.well-known/agent.json
